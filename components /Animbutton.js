...
export default class Animbutton extends Component {
  ...
  _onPress() {
    ...
    console.log("for btn", this.state.status)
  ...
  render() {
    return (
      <TouchableWithoutFeedback onPress={() => this._onPress()}>
        <Animatable.View  ref="view" style={{ margin: 10, paddingTop: 10, paddingBottom: 10, paddingRight: 20, paddingLeft: 20, backgroundColor: this.state.status === true ? this.props.onColor :"red"  , borderRadius: 20 }}>
          <Text style={{ color: this.state.status ? "black" : "black", fontWeight: "bold" }}>{this.props.text} </Text>
        </Animatable.View>
      </TouchableWithoutFeedback>
    );
  }
